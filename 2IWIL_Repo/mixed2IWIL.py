...
parser.add_argument('--traj-size', type=int, default=4000)
...
try:
    ...
    mixtext_output = np.load('split_sorted_confidences.npy')
except:
    print('Mixture demonstrations not loaded successfully.')
    assert False

idx = mixtext_output[0].astype(int)
print(idx)
...
# ###################################
expert_conf = mixtext_output[1].reshape((-1,1))
print(expert_conf) 
Z = expert_conf.mean()
if args.only:
    fname = 'olabel'
